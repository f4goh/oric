...
void setup( )
{
  ...
  //Timer1.attachInterrupt(scanOricKeyboard);
  Timer1.attachInterrupt(arrowGame);
...
//A conserver pour test ultérieur
...
void onlyCar(byte code) {
  ...
  while (millis() - start < TIME_PRESS_Z);
  digitalWrite(KEY_EN, LOW);
}
...
void touchesSimples(byte key) {
  ...
  if ((key >= 'A' && key <= 'Z') || (key >= '0' && key <= '9')) { //problème avec le caps lock et le shift
    ...
    if (key == 'Z') {   //possible d'afficher le Z sous irq simple... onycar a revoir.....changer l'irq
      onlyCar(code);
      releaseKey();
      notSupported = true;
    }
...
void touchesSimples(byte key) {
  ...
  if (key >= 'A' && key <= 'Z') { //problème avec le caps lock et le shift
    ...
    //code = oricAsciiCode[key - ' '];
    if (key == 'W') {   //possible d'afficher le Z sous irq simple... onycar a revoir.....changer l'irq
      onlyCar(code);
      releaseKey();
      notSupported = true;
    }
    ...
    switch (key) {
      ...
      case PS2_KEY_7:   code = 0x40;
        break;
      case PS2_KEY_8:   code = oricAsciiCode['_' - ' '];
        break;
...
void touchesShift(byte key) {
  ...
  if ((key >= 'A' && key <= 'Z')) { //+Shift pour les caractères
    ...
    switch (key) {
      ...
      case PS2_KEY_EQUAL :   code = oricAsciiCode['+' - ' '];
        break;
      case PS2_KEY_EUROPE2:   code = oricAsciiCode['>' - ' '];    //facultatif
        break;
...
void loop( )
{
  ...
  {
    Timer1.attachInterrupt(scanOricKeyboard); //avant le readStringUntil.
