...
function SimulatorWidget(node) {
  ...
  function initialize() {
    ...
    $node.find('.downloadButton').click(display.downloadTap);
  ...
  function UI() {
    ...
    function setState(state) {
      ...
      $node.find('.downloadButton').attr('disabled', !state.run[0]);   
      currentState = state;
    }
  ...
  function Display() {
    var displayArray = [];
    var palette = [
      "#000000", "#ff0000", "#00ff00", "#ffff00", //black,red,green,yellow
      "#0000ff", "#ff00ff", "#00ffff", "#ffffff"  //blue,magenta,cyan,white
    ];
    ...
    function reset() {
      ...
      var adBase;
      var x;
      for (let ad = 0xbb80; ad < 0xbfe0; ad++) {    
            adBase=ad-0xbb80;
            x=adBase%40;
            if (x===0){
                memory.set(ad,23);
      ...
      memory.set(0xbb80,16);
      memory.set(0xbb81,7);
      memory.set(0xbbA4,'C'.charCodeAt());
      memory.set(0xbbA5,'A'.charCodeAt());
      memory.set(0xbbA6,'P'.charCodeAt());
      memory.set(0xbbA7,'S'.charCodeAt());      
      updatePixel(0xbb80);
    }
        
        function updateChar(posx,posy,car,indexCoul) {
            ctx.fillStyle = palette[indexCoul];
            var y = posy*8;
            var x = posx*6;

            // Loop through each row of the character set for 'A' (ASCII code 65)
            for (var i = 0; i < 8; i++) {
                //var rowData = charset[(car-0x20) * 8 + i]; // Get the corresponding row data from the character set
                var rowData = memory.get((car-0x20) * 8 + i+ 0xb500); // Get the corresponding row data from the character set

                // Loop through each bit in the row data
                for (var j = 0; j < 6; j++) {
                    var pixelValue = (rowData >> (5 - j)) & 1; // Extract the j-th bit value

                    // Check if the pixel is set (1) and fill the corresponding rectangle
                    if (pixelValue === 1) {
                ...
                x = posx*6; // Reset x to start from the beginning of the row
                y++; // Move to the next row
            }
        }

        function updatePaper(posx,posy,indexCoul) {
            ctx.fillStyle = palette[indexCoul];
            var y = posy*8;
            var x = posx*6;
            ctx.fillRect(x * pixelSize, y * pixelSize, 6, 8);
        }
    ...
    function updatePixel(addr) {
      ...
      for (let ad = 0xbb80; ad < 0xbfe0; ad++) {    
            ...
            updatePaper(x,y,coulPaper);
            if (code>=32 && code <=127){
                updateChar(x,y,code,coulInk);    
            }
      }
        ...
        function downloadTap() {
            ...
            var header =[0x16,0x16,0x16,0x24,0x00,0x00,0x80,0xC7,0x04,0x1A,0x04,0x00,0x00];
            
            var texte = "FILE";

            texte += '\0';

            // Utiliser TextEncoder pour convertir la chaîne en tableau d'octets (Uint8Array)
            var encodeur = new TextEncoder();
            var tableauOctets = encodeur.encode(texte);
           
            var uint8Array = memory.getSource(0x600);
            var adFin = 0x600 + uint8Array.length;
            header[8]=(adFin >> 8) & 0xFF;
            header[9]=(adFin & 0xFF);
            header[10]=(0x600 >> 8) & 0xFF;
            header[11]=(0x600 & 0xFF);

            // Concaténer les trois tableaux
            var tableauConcatene = new Uint8Array(header.length + tableauOctets.length + uint8Array.length);
            tableauConcatene.set(header);
            tableauConcatene.set(tableauOctets, header.length);
            tableauConcatene.set(uint8Array, header.length + tableauOctets.length);
            ...
            var blob = new Blob([tableauConcatene], {type: 'application/octet-stream'});
            // Utilisation de window.URL pour créer un objet URL
            var url = window.URL.createObjectURL(blob);
            // Création d'un élément <a> pour déclencher le téléchargement
            var a = document.createElement('a');
            a.href = url;
            a.download = 'fichier.tap';
            // Ajout de l'élément <a> au DOM
            document.body.appendChild(a);
            // Clic sur l'élément <a> pour déclencher le téléchargement
            a.click();
            // Suppression de l'élément <a> du DOM
            document.body.removeChild(a);
            // Révocation de l'URL pour libérer les ressources
  ...
  function Memory() {
    var memArray = new Array(0x10000);
        ...
        async function loadBinaryFile(url) {
            try {
                ...
                const romArray = new Uint8Array(data);

                // Copie le contenu du fichier binaire dans memArray
                for (let i = 0; i < romArray.length; i++) {
                    memArray[i+0xc000] = romArray[i];
                }
                //b500 à b800  : 300  fc78 goto b500  / ff70 goto b7f8  
                //b900 à bb00  : 
                
                for (let i = 0; i < 0x300; i++) {
                    memArray[i+0xb500] = memArray[i+0xfc78];
                }
                //init text memory
                
                //ajouter charset 2
                console.log("Fichier binaire chargé avec succès dans memArray.");
            } catch (error) {
                console.error("Erreur lors du chargement du fichier binaire :", error);
            }
        }
        loadBinaryFile('basic11b.rom');
    ...
    function getSource(start) {
      var codeLen = assembler.getCodeLen();
      var uint8Array = new Uint8Array(memArray.slice(start, start+codeLen));
      return uint8Array;
    }
